...
def send_to_sqs(
    queue_url: str, message_body: str, delay: int = 10, message_atr: dict = {}
) -> dict:
    """
...
class LambdaEventMapping:
    ...
    def get_lambda_event_source(cls, event: dict) -> str:
        ...
        if cls.is_lambda_event(event):
            # any dictionary that does not map to one of the above will be a lambda event
            return EventMappingEnum.LAMBDA
    ...
    def is_dynamo_db_event(event: dict) -> bool:
        ...
        return (
            ...
            and len(event["Records"]) > 0
            and "eventSource" in event["Records"][0]
            and event["Records"][0]["eventSource"] == "aws:dynamodb"
        )
    ...
    def is_aws_config_event(event: dict) -> bool:
        ...
        """
        return (
            "configRuleId" in event
            and "configRuleName" in event
            and "configRuleArn" in event
        )
    ...
    def is_code_commit_event(event: dict) -> bool:
        ...
        return (
            ...
            and len(event["Records"]) > 0
            and "eventSource" in event["Records"][0]
            and event["Records"][0]["eventSource"] == "aws:codecommit"
        )
    ...
    def is_kinesis_event(event: dict) -> bool:
        ...
        return (
            ...
            and len(event["Records"]) > 0
            and "eventSource" in event["Records"][0]
            and event["Records"][0]["eventSource"] == "aws:kinesis"
        )
    ...
    def is_kinesis_firehose_event(event: dict) -> bool:
        ...
        return (
            "records" in event
            and len(event["records"]) > 0
            and "approximateArrivalTimestamp" in event["records"][0]
        ) or (
            "records" in event
            and len(event["records"]) > 0
            and "deliveryStreamArn" in event
            and event["deliveryStreamArn"] is str
            and event["deliveryStreamArn"].startswith("arn:aws:kinesis:")
        )
    ...
    def is_cognito_sync_trigger_event(event: dict) -> bool:
        ...
        return (
            "eventType" in event
            and event["eventType"] == "SyncTrigger"
            and "identityId" in event
            and "identityPoolId" in event
        )
    ...
    @staticmethod
    def is_lambda_event(event: dict) -> bool:
        """
        Determines if event is lambda
        """
